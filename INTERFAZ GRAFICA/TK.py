...
def MDesplegable(): #TERMINAR
    if MDesplegable == "/dev/ttyACM0":
        Puerto = "/dev/ttyACM0"
    if MDesplegable == "COM8":
        Puerto = "COM8"
    if MDesplegable == "/dev/ttyS0":
        Puerto = "/dev/ttyS0"
    return Puerto
...
MDesplegable = ttk.Combobox(armControl, state="readonly",   #TERMINAR MENU DESPLEGABLE FALTA DEFINICION DE FUNCION
values=[
        "/dev/ttyACM0",
        "COM8",
        "/dev/ttyS0"])
MDesplegable.set("Selecciona Puerto")
MDesplegable.pack()
